...
export default function FeeTypeCollection() {
    ...
    try {
      ...
      if (response.data.success) {
        const formattedData = response.data.data.map((item, index) => ({
          ...
          invoicedAmount: `₹${item.invoicedAmount.toLocaleString()}`,
          collectedAmount: `₹${item.collectedAmount.toLocaleString()}`,
  ...
  const handleExportCSV = async () => {
    ...
    try {
      ...
      link.href = url;
      link.download = `fee-type-collection-${new Date().toISOString().split('T')[0]}.csv`;
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600 dark:text-gray-300">Track collection by fee type</p>
            </div>
          </div>
          <button
          ...
          <div className="p-4 bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-700 dark:to-gray-800 border-b border-gray-200 dark:border-gray-600">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-semibold text-gray-900 dark:text-white">Fee Type Collection Results</h3>
                <span className="px-2 py-1 bg-emerald-100 dark:bg-emerald-900 text-emerald-800 dark:text-emerald-200 text-xs rounded-full font-medium">
                  {data.length} records
                </span>
              </div>
            </div>
